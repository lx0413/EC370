...
E-mail: $if(email)$\texttt{$email$}$else$TBD$endif$ & Course Web: $if(web)$\href{http://$web$}{\tt $web$}$else$TBD$endif$\\
Office Hours: $if(officehours)$$officehours$$else$TBD$endif$  &  Class Hours: $if(classhours)$$classhours$$else$TBD$endif$\\
Virtual Office: $if(office)$$office$$else$TBD$endif$  & Classroom: $if(classroom)$$classroom$$else$TBD$endif$\\
	&  \\
	\hline
	\end{tabular*}
